...
import mapboxgl, { LngLat, Map as MapGl, Marker } from 'mapbox-gl';
...
const PlayGame = ({ setlngToQuestion, setlatToQuestion , click }: PropsSetlingSetlat) => {
  ...
  const markerQuizRef = useRef<mapboxgl.Marker[]>([])
  ...
  const showQuestionsOnMap = (questions: QuizResponseQuestions[]) => {
    // användaren har klickat på att visa frågor för ett quiz
    // eventuella gamla markers ska tas bort
    markerQuizRef.current.forEach(marker => {
      marker.remove()
    })
    markerQuizRef.current = []
  ...
  useEffect(() => {
    ...
    selectedQuestions.forEach(question => {
      ...
      //denna funkade inte 
      // if(markerQuizRef.current){ markerQuizRef.current.remove().addTo(mapGL.current)}
      ...
      const newMarker: Marker = new mapboxgl.Marker({color:'#d6bd8b'})
      newMarker.setLngLat([Number(question.location.longitude), Number(question.location.latitude)])
      newMarker.addTo(mapGL.current);
      newMarker.setPopup(new mapboxgl.Popup().setHTML(`<h1>Fråga: ${ question.question } Svar: ${ question.answer}</h1>`))
      .addTo(mapGL.current)
      markerQuizRef.current.push(newMarker)
    })
  }, [selectedQuestions])
